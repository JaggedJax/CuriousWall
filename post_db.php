...
if ($_POST['method'] == 'get') {
	...
	if (!isset($_POST['begin']))
		$_POST['begin'] = '';
	post_get($db, $_POST['topic'], $_POST['begin']);
	die();
}
	...
	if (isset($_POST['stick']) || isset($_POST['lock'])) {
		if ((!isset($_SESSION['permissions']) || ($_SESSION['permissions'] == 0))) {
			die('You need to be a moderator to play with sticky or locked topics!');
		} else {
	...
	if (isset($_POST['stick']) && $_POST['stick'] == 'ystick') {
		$_POST['stick'] = '1';
	} elseif (isset($_POST['stick'])) {
		$_POST['stick'] = '0';
	} else {
		
	}
	if (isset($_POST['lock']) && $_POST['lock'] == 'ylock') {
		$_POST['lock'] = '1';
	...
	if ($_POST['topic'] < 0) {
		...
		if ($row && $row['locked'] == 1) {
			if ((!isset($_SESSION['permissions']) || ($_SESSION['permissions'] == 0)))
				die("You can not post in a locked topic unless you are a moderator!");
		}
