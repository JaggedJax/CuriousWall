...
function theusername($name, $permissions) {
	if ($permissions == 1) {
		echo $name . "♦" . ((isset($bio)) ? "<div class='bio'>" . $bio . "</div>" : '');
	} else
		echo $name . ((isset($bio)) ? "<div class='bio'>" . $bio . "</div>" : '');
}
...
function displaytopiclocked($topicname, $islocked) {
	if ($islocked == 1)
		echo $topicname . ' <i class="icon-lock"></i>';
	else
		echo $topicname;
}
...
function displaymenu($current) {
	if ($current == 'topics') {
		echo "<div id='navbar'><div id='topicsnav currentnav'>Topics</div><div id='usernav'>Users</div></div>";
	} else {
		echo "<div id='navbar'><div id='topicsnav'>Topics</div><div id='usernav currentnav'>Users</div></div>";
	}
}
