	...
	<div id="left" class="box wall">
		...
		<div id="user_panel" style="margin-top:12px;margin-bottom:8px">
			...
			if ((isset($_SESSION['user_name'])) && (isset($_SESSION['permissions']))) {
				...
				theusername($_SESSION['user_name'], $_SESSION['permissions']);
...
<?php /* display mod tools only to mods */ if ((isset($_SESSION['permissions']) && ($_SESSION['permissions'] == 1))) {
	echo '<div id="stickingswitch" class="radioswitch"><input type="radio" id="nostick" class="inputsticky" name="stick" value="nostick"><label for="nostick">Unstick</label><input type="radio" id="ystick" class="inputsticky" name="stick" value="ystick"><label for="ystick">Stick</label></div>';
} else {
	
} ?>
<?php /* locking switch */ if ((isset($_SESSION['permissions']) && ($_SESSION['permissions'] == 1))) {
	echo '<div id="lockingswitch" class="radioswitch"><input type="radio" id="nolock" class="inputlocked" name="lock" value="nolock"><label for="nolock">Unlock</label><input type="radio" id="ylock" class="inputlocked" name="lock" value="ylock"><label for="ylock">Lock</label></div>';
} else {
