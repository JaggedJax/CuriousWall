...
function post_get($db, $xtopic, $xbegin) {
	if (is_numeric($xtopic) && (is_numeric($xbegin))) {
		...
		. ' begin=' . ($begin + 1) . ' end=' . ($end + 1) . ' limit=' . $limit . '>';
		displaytopiclocked($topic['topic_title'], $topic['locked']);
		echo'</div>';
		echo '<div id="topic_text">' . "<span class='username' style='font-weight: bold;'>";
		theusername($topic['user_name'], $topic['permissions']);
		echo ":</span> " . $topic['topic_text'] . '<div class="postbuttons">';
		...
		if ($topic['locked'] == '1') {
			$isitlockeddisplaymessage = true;
		} else {
			$isitlockeddisplaymessage = false;
		}
		...
		while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
			...
			echo "<span class='username' style='font-weight: bold;'>";
			theusername($row['user_name'], $row['permissions']);
			echo ":</span> ";
			echo $row['post_text'] . '<div class="postbuttons">';
			if ((isset($_SESSION['permissions']) && ($_SESSION['permissions'] == 1))) {
				echo'<div id="' . $row['post_id'] . '-' . ($i + 1) . '" class="delete postdelete hover" title="delete">⨯</div>';
			}echo'</div></div>';
			$i++;
		}
		...
		while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
			...
			echo '<p>';
			displaytopiclocked($row['topic_title'], $row['locked']);
			echo ((($row['sticky']) == 1) ? ' <i class="icon-pushpin"></i>' : '') . '</p></div>';
			$i++;
		}
		return $first_topic;
	}
